...
/datum/action/innate/cult/blood_spell/horror/do_ability(mob/living/user, mob/living/carbon/human/clicked_on)
	...
	to_chat(user, span_cult_bold("[clicked_on] has been cursed with living nightmares!"))
		...
		if(ishuman(target))
			...
			if(IS_CULTIST(H))
				...
				if(H.blood_volume > BLOOD_VOLUME_SAFE)
					...
					to_chat(user,span_cult_italic("Your blood rite gains 50 charges from draining [H]'s blood."))
					new /obj/effect/temp_visual/cult/sparks(get_turf(H))
				else
