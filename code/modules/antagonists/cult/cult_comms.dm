...
/datum/action/innate/cult/master/pulse/do_ability(mob/living/user, atom/clicked_on)
	...
	if(throwee)
		...
		if(isliving(clicked_on))
			...
			to_chat(user, span_cult_bold("You reach through the veil with your mind's eye and seize [clicked_on]! <b>Click anywhere nearby to teleport [clicked_on.p_them()]!</b>"))
			throwee_ref = WEAKREF(clicked_on)
			return TRUE

		if(istype(clicked_on, /obj/structure/destructible/cult))
			to_chat(user, span_cult_bold("You reach through the veil with your mind's eye and lift [clicked_on]! <b>Click anywhere nearby to teleport it!</b>"))
			throwee_ref = WEAKREF(clicked_on)
			return TRUE
