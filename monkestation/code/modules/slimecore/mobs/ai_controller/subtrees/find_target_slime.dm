...
/datum/ai_planning_subtree/slime_find_non_latched_target/SelectBehaviors(datum/ai_controller/controller, seconds_per_tick)
	if(!controller.blackboard[BB_BASIC_MOB_STOP_FLEEING])
		return
	controller.queue_behavior(/datum/ai_behavior/find_potential_targets/slime_non_latched_target, BB_SLIME_CURRENT_TARGET_MOB, BB_SLIME_NON_LATCHED_TARGETING_STRATEGY, BB_BASIC_MOB_CURRENT_TARGET_HIDING_LOCATION)
...
/datum/ai_planning_subtree/slime_find_non_latched_target_and_smaller/SelectBehaviors(datum/ai_controller/controller, seconds_per_tick)
	if(!controller.blackboard[BB_BASIC_MOB_STOP_FLEEING])
		return
	controller.queue_behavior(/datum/ai_behavior/find_potential_targets/slime_non_latched_target_and_smaller, BB_SLIME_CURRENT_TARGET_MOB, BB_SLIME_NON_LATCHED_AND_SMALLER_TARGETING_STRATEGY, BB_BASIC_MOB_CURRENT_TARGET_HIDING_LOCATION)
